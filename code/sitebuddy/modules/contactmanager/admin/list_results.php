...
foreach($filters as $key => $value){
	if($search_query=="") 
		...
		$search_query .= " AND (";
	$search_query .= generateSearchSQL($value["cnd"],$value["txt"],$value["not"],$value["switch"]);
	$search_query .= ")";
}
...
//Add all contacts to a list user selects option to add results to a group
if($groupid != ""){
	mysql_query("INSERT INTO sb_cnt_grouplist SELECT contactid, ".$groupid." FROM sb_contacts c LEFT JOIN (SELECT * from sb_cnt_grouplist WHERE groupid=".$groupid.") gl using(contactid)".$search_query." AND groupid is null;");
}
?>
...
?>    <td><div class="editIcon" onclick="editContact(<?php echo($contact->contactid); ?>)"></div></td>
    <td><div class="deleteIcon" onclick="delContact(<?php echo("'".$contact->pre." ".$contact->first." ".$contact->last."',".$contact->contactid); ?>)"></div></td>
  </tr>
<?php }
} else {?>
No Results Found
